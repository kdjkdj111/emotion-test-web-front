...
const LargeUploadView = ({
    ...
    // 큰 이모티콘 규격: 16개 세트 구성 (대형 이미지 특성 고려)
    const MAX_SLOTS = 16;
            ...
            <div className="font-studio flex h-full w-full">
                ...
                <div className="w-3/5 border-r border-slate-200 p-8 flex flex-col bg-slate-50 h-full">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    <h1 className="text-2xl font-extrabold text-slate-800 tracking-tighter">큰 이모티콘</h1>
                                    <span className="px-2 py-0.5 bg-cyan-100 text-cyan-700 text-[10px] font-bold rounded-md">LARGE</span>
                                </div>
                                <p className="text-[11px] font-bold text-slate-400 mt-0.5">가변 규격 (최대 540px) | 16개 구성을 확인하세요.</p>
                            </div>
                        </div>
                    </header>
                    ...
                    >
                        ...
                        <p className={`font-bold text-base transition-colors ${isDragging ? 'text-cyan-700' : 'text-slate-700'}`}>
                            ...
                            >
                                {previews[i] ? (
                                    <>
                                        ...
                                        <div className="absolute -top-3 -left-3 bg-cyan-600 text-white text-[10px] font-extrabold rounded-full w-7 h-7 flex items-center justify-center shadow-md z-10 border-2 border-white">{i + 1}</div>
                ...
                <div className="w-2/5 bg-white p-10 flex flex-col h-full shadow-2xl z-10">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-[11px] text-slate-400 font-bold mt-1 uppercase tracking-wider">Resolution & Canvas Check</p>
                        </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center space-x-3 truncate text-slate-700 font-bold">
                                                <span className="text-cyan-600 font-extrabold w-5">{i + 1}</span>
                                                <span className="truncate text-sm">{item.name}</span>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                {res.status === 'LOADING' && <div className="w-4 h-4 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin"></div>}
                                                {res.status === 'SUCCESS' && <span className="text-cyan-600 font-bold text-[10px] bg-cyan-50 px-2 py-1 rounded-md border border-cyan-100">통과</span>}

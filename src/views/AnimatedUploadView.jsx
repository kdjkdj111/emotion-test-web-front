                ...
                <div className="w-3/5 border-r border-slate-200 p-8 flex flex-col bg-slate-50 h-full">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    <h1 className="text-2xl font-extrabold text-slate-800 tracking-tighter">움직이는 이모티콘</h1>
                                    <span className="px-2 py-0.5 bg-indigo-100 text-indigo-600 text-[10px] font-bold rounded-md">GIF/APNG</span>
                                </div>
                                <p className="text-[11px] font-bold text-slate-400 mt-0.5">360 x 360 px | 최대 24개 구성을 확인하세요.</p>
                            </div>
                        </div>
                    </header>
                    ...
                    <div
                        ...
                        className={`flex-shrink-0 border-2 border-dashed rounded-[2rem] p-8 text-center transition-all cursor-pointer mb-8 group shadow-sm
                            ${isDragging
                            ? 'border-indigo-400 bg-indigo-50/50 scale-[1.01] shadow-lg ring-4 ring-indigo-400/10'
                    ...
                    >
                        ...
                        <p className={`font-bold text-base transition-colors ${isDragging ? 'text-indigo-700' : 'text-slate-700'}`}>
                            {isDragging ? '애니메이션 파일 놓기' : '애니메이션 파일 추가하기'}
                        </p>
                        <p className="text-[11px] text-slate-400 mt-1 font-medium">
                            {isDragging ? '마우스 버튼을 놓으세요' : '파일당 최대 2MB 이내 권장 (드래그 지원)'}
                        </p>
                    </div>

                    {/* 그리드: 24구 구성 */}
                    <div className="grid grid-cols-4 gap-x-5 gap-y-10 overflow-y-auto flex-1 px-3 py-4 content-start min-h-0 scrollbar-hide">
                        {[...Array(MAX_SLOTS)].map((_, i) => (
                            <div key={`animated-slot-${i}`}
                                 ...
                                 ${previews[i] ? 'cursor-zoom-in hover:border-indigo-400 hover:shadow-md' : 'hover:border-slate-300'}`}
                            >
                                {previews[i] ? (
                                    <>
                                        ...
                                        <div className="absolute -top-3 -left-3 bg-indigo-500 text-white text-[10px] font-extrabold rounded-full w-7 h-7 flex items-center justify-center shadow-md z-10 border-2 border-white">{i + 1}</div>
                                        <button onClick={(e) => { e.stopPropagation(); handleRemoveFile(i); }} className="absolute -top-2.5 -right-2.5 bg-slate-800 text-white rounded-full w-6 h-6 text-[10px] flex items-center justify-center shadow-md z-20 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500">✕</button>
                                    </>
                                ) : (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-200 bg-slate-50/50 rounded-2xl">
                ...
                <div className="w-2/5 bg-white p-10 flex flex-col h-full shadow-2xl z-10">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-[11px] text-slate-400 font-bold mt-1 uppercase tracking-wider">Frame & Frame Analysis</p>
                        </div>
                                    ...
                                    return (
                                        <div key={item.id}
                                             ...
                                             className={`flex items-center justify-between p-4 rounded-2xl border shadow-sm transition-all cursor-pointer hover:shadow-md hover:scale-[1.02]
                                                ${res.status === 'FAILED' ? 'bg-red-50 border-red-200' : 'bg-white border-slate-100 hover:border-indigo-300'}`}
                                        >
                                            <div className="flex items-center space-x-3 truncate text-slate-700 font-bold">
                                                <span className="text-indigo-500 font-extrabold w-5">{i + 1}</span>
                                                <span className="truncate text-sm">{item.name}</span>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                {res.status === 'LOADING' && <div className="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div>}
                                                {res.status === 'SUCCESS' && <span className="text-indigo-500 font-bold text-[10px] bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100">통과</span>}
...
export default AnimatedUploadView;
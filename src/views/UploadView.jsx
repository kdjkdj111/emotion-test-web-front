...
const UploadView = ({
    ...
    return (
        <div className="flex h-full w-full overflow-hidden text-left font-sans tracking-tight">
            ...
            {/* ⭐ [FIX] font-friendly를 font-studio로 수정하여 서체를 활성화합니다. */}
            <div className="font-studio flex h-full w-full">
                ...
                <div className="w-3/5 border-r border-slate-200 p-8 flex flex-col bg-slate-50 h-full">
                    <header className="mb-10 flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-extrabold text-slate-800 tracking-tighter">이모티콘 업로드</h1>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">파일 업로드 및 분석</p>
                        </div>
                    </header>
                    ...
                    <div onClick={handleUploadClick} className="flex-shrink-0 border-2 border-dashed border-slate-300 bg-white rounded-3xl p-10 text-center hover:border-yellow-400 hover:bg-yellow-50 transition-all cursor-pointer mb-10 group shadow-sm">
                        ...
                        <p className="text-slate-700 font-bold text-lg">새로운 이미지 추가하기</p>
                    </div>
                            ...
                            >
                                {previews[i] ? (
                                    <>
                                        ...
                                        <div className="absolute -top-3 -left-3 bg-yellow-400 text-white text-[11px] font-extrabold rounded-full w-8 h-8 flex items-center justify-center shadow-md z-10 border-2 border-white">{i + 1}</div>
                                ...
                                ) : <div className="absolute inset-0 flex items-center justify-center text-slate-200 font-extrabold text-2xl bg-slate-50/50 rounded-2xl">{i + 1}</div>}
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="w-2/5 bg-white p-10 flex flex-col h-full shadow-2xl z-10">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        {files.length > 0 && (
                            <button
                                onClick={handleReset}
                                className="text-[11px] font-bold text-red-400 hover:text-red-600 transition-colors flex items-center gap-1 bg-red-50 px-3 py-1.5 rounded-full"
                            >
                    ...
                    <div className="flex-1 bg-slate-50 border border-slate-200 rounded-3xl p-6 overflow-y-auto mb-8 shadow-inner">
                        {files.length === 0 ? <p className="text-slate-300 text-center mt-20 font-bold">업로드된 파일이 없어요.</p> : (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center space-x-2">
                                                {res.status === 'LOADING' && <span className="text-slate-400 text-xs animate-pulse font-bold">분석 중...</span>}
